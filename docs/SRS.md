...
|---|---|
| **Source** | Trang web nguồn cung cấp truyện gốc duy nhất (dilib.vn). |
| **Bot Config** | Bộ cấu hình selectors (CSS Selectors) và các quy tắc kiểm duyệt bản dịch cho nguồn. |
...
| **Worker** | Tiến trình Node.js độc lập nhận job từ Redis Queue để cào, xử lý ảnh và đẩy lên CDN. |
...
1.  **Quản trị viên (Admin)** thiết lập các CSS Selectors tương ứng trong **Bot Config** cho nguồn cào duy nhất.
2.  Khi thêm một **Truyện (Story)**, Admin nhập URL gốc thuộc nguồn `dilib.vn`, chọn Bot Config phù hợp và cài đặt tần suất quét tự động (Cron Schedule) cho truyện đó.
...
*   **Độc giả (End User):** Chỉ đọc truyện tranh trên User Portal, không cần đăng nhập.
...
*   **UR-1.4 Đọc nội dung chương:** Hiển thị danh sách ảnh dọc, tải mượt mà từ CDN Cloudinary.
...
*   **UC-2: Quản lý Bộ cấu hình Bot (Bot Configs Management)**
    *   **Tác nhân:** Admin
    *   **Điều kiện tiên quyết:** Có CSS Selector từ trang nguồn.
    *   **Luồng sự kiện chính:**
        1. Admin vào trang Bot Configs, bấm "Thêm Bot".
        2. Hệ thống hiển thị form nhập liệu.
        3. Admin nhập `layoutName` (VD: "Dilib Mobile").
    ...
    *   **Luồng ngoại lệ:**
        *   Thiếu trường bắt buộc (VD: không nhập `imageSelector`): Hệ thống highlight ô lỗi và từ chối lưu.
    *   **Kết quả:** Bot Config sẵn sàng để sử dụng cho các bộ truyện.

*   **UC-3: Quản lý Truyện và Lập lịch cào (Story & Scheduling Management)**
    *   **Tác nhân:** Admin
    *   **Điều kiện tiên quyết:** Đã có Bot Config (UC-2) được cấu hình.
    *   **Luồng sự kiện chính:**
        1. Admin vào trang Truyện, bấm "Thêm truyện mới".
        2. Admin dán URL gốc của truyện (trang mục lục thuộc domain dilib.vn).
    ...
    *   **Luồng ngoại lệ:**
        ...
        *   URL truyện không thuộc domain dilib.vn: Hệ thống báo lỗi tên miền không khớp.
    *   **Kết quả:** Truyện được thêm vào DB. Scheduler bắt đầu tự động theo dõi truyện này.

*   **UC-4: Điều khiển tác vụ Cào thủ công (Manual Crawl Trigger)**
    *   **Tác nhân:** Admin
...
*   **UC-5: Giám sát Hệ thống và Nhật ký (Logs & Monitoring)**
    *   **Tác nhân:** Admin
    ...
    *   **Luồng sự kiện chính:**
        ...
        5. Admin nhận tin nhắn Telegram, vào Dashboard sửa lại CSS (quay lại UC-2).
...
*   **UR-3.2 Xử lý và Tối ưu hóa dữ liệu chương (Comic Pipeline):** Tải ảnh gốc từ nguồn (gắn Referer bypass 403) -> Dùng thư viện Sharp để nén chất lượng giảm dung lượng (chất lượng 75%) và chuyển sang định dạng WebP -> Upload lên Cloudinary CDN -> Lưu danh sách CDN URLs.
