...
*   **Mục tiêu:** Xây dựng hệ thống tự động cào (crawl) truyện tranh từ nguồn trên web và quản trị tập trung.
...
*   **Phân hệ chính:**
    1. **User Portal:** Web đọc truyện tranh hiển thị dọc.
    ...
    4. **Crawl Worker:** Tiến trình Node.js không trạng thái (stateless) nhận Job từ Redis để cào, xử lý ảnh và gửi dữ liệu về API Server.
...
*   So sánh với DB: Các chương nào chưa có trong DB sẽ được bóc tách và đẩy Job `FETCH_CHAPTER` vào hàng đợi tải ảnh.
...
*   So sánh với `latestChapterHash` trong DB:
    *   **Khác nhau:** Nguồn đã cập nhật nội dung thầm lặng (VD: up bản dịch đè bản Raw cũ). Worker lập tức tải ảnh mới, nén và gọi API Upsert để ghi đè dữ liệu mới tinh, mang lại bản dịch hoàn chỉnh cho người dùng.
    *   **Giống nhau:** Bỏ qua.
...
Hệ thống có 6 bảng chính: `users`, `genres`, `bot_configs`, `stories`, `chapters`, `crawl_logs`.

### Các thiết kế đáng chú ý:
*   **Quan hệ 1:N linh hoạt:** Một `bot_configs` (CSS Selectors của 1 layout) có thể được dùng chung cho hàng ngàn `stories` của nguồn.
...
*   **Rate Limiting:** Sử dụng tính năng giới hạn tốc độ (Rate Limiter) của BullMQ được cấu hình riêng cho Queue cào duy nhất để tránh web nguồn block IP.
